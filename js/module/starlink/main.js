import { updateInformationBelowstarlink } from "./down-info/downinfo.js";
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    buttonsContainer.addEventListener('click', async (event) => {
        if (event.target && event.target.nodeName === 'BUTTON') {
            const starlinkId = event.target.id; // Obtener el ID del botón clickeado

            // Llamar a las funciones para actualizar el contenido con el ID del cohete
            await updateInformationBelowstarlink(starlinkId);
        }
    });
