...
// Función para manejar la selección en el footer
const footerSelect = async (e, id) => {
    e.preventDefault();
    // Opcional: Realiza alguna operación de carga o preparación aquí, si es necesario
    let liElements = document.querySelectorAll(".footer_Space_X ul li");
    liElements.forEach(li => {
        li.querySelector("a").classList.remove('select');
    });
    id.querySelector("a").classList.add('select');
};

// Función para manejar el clic en las secciones del footer
const handleFooterItemClick = async (e) => {
    const clickedLi = e.currentTarget;
    await footerSelect(e, clickedLi);

    const buttonsContainer = document.querySelector(".buttons");
    buttonsContainer.innerHTML = ""; // Limpiar el contenido actual
    buttonsContainer.append(await menubuttonsClickeable()); // Generar y añadir nuevos botones dinámicamente
};

// Obtener todos los elementos <li> del footer
const footerItems = document.querySelectorAll(".footer_Space_X ul li");

// Agregar evento de clic a cada elemento <li> del footer
footerItems.forEach(item => {
    item.addEventListener("click", handleFooterItemClick);
});

// Ejemplo de uso específico para actualizar títulos y texto al cargar la página
document.addEventListener('DOMContentLoaded', () => {
    updateTitleByRocketId('5e9d0d95eda69955f709d1eb'); // ID de ejemplo
    updateLeftTextsRocket('5e9d0d95eda69955f709d1eb');
});

// Ejemplo adicional para manejar eventos de clic en botones generados dinámicamente
document.addEventListener('DOMContentLoaded', () => {
    const buttonsContainer = document.querySelector('.buttons');
    buttonsContainer.addEventListener('click', (event) => {
        const clickedButton = event.target.closest('.button_main_page');
        if (clickedButton) {
            event.preventDefault(); // Prevenir el comportamiento por defecto del enlace
            console.log('Botón clickeado:', clickedButton.textContent.trim());
            // Aquí puedes agregar la lógica adicional que necesites al hacer clic en los botones generados dinámicamente
        }
    });
});
